...
static int gs_reclient_enable = FALSE;
...
BOOL gInitialize()
{
	...
	{
		KGLogPrintf(LOG_INFO,"FAIL: relay server\n");
		//return FALSE;
	}
	...
	{
		KGLogPrintf(LOG_INFO,"FAIL: root client - %s\n", addrRoot.c_str());
	...
	{
		KGLogPrintf(LOG_INFO,"FAIL: DBRole client - %s\n", addrDB.c_str());
	...
	{
		KGLogPrintf(LOG_INFO,"FAIL: gateway client - %s\n", addrGateway.c_str());
	...
	{
		KGLogPrintf(LOG_INFO,"FAIL: host server\n");
		//return FALSE;
	}
	...
	{
		KGLogPrintf(LOG_INFO,"FAIL: tong server\n");
		//return FALSE;
	}
	...
	{
		KGLogPrintf(LOG_INFO,"FAIL: chat server\n");
		//return FALSE;
	}
	...
	if (g_cTongDB.Open())
	{
		...
		if (g_cTongSet.InitFromDB())
			KGLogPrintf(LOG_INFO,"tongset init ok !\n");
		else
			KGLogPrintf(LOG_INFO,"FAIL: tongset init error!!!\n");
	}
	else
	{
		KGLogPrintf(LOG_INFO,"FAIL: tong database open error!!!\n");
	}
...
BOOL gOnOneClientCreate(ONECLIENT ct)
{
	...
	case oneclient_root:
		if (gs_timerRoot)
		{
			...
			{
				gs_timerRoot = 0;
				KGLogPrintf(LOG_INFO,"disable reclient: root\n");
			}
		}
		break;
	case oneclient_gateway:
		if (gs_timerGateway)
		{
			...
			{
				gs_timerGateway = 0;
				KGLogPrintf(LOG_INFO,"disable reclient: gateway\n");
			}
		}
		break;
	case oneclient_db:
		if (gs_timerDB)
		{
			...
			{
				gs_timerDB = 0;
				KGLogPrintf(LOG_INFO,"disable reclient: db\n");
			}
		}
		break;
	default:
	...
	case oneclient_root:
		if (!gs_timerRoot)
		{
			...
			if (rootinterval > 0)
			{
				...
				if (!gs_timerRoot)
				{
					KGLogPrintf(LOG_INFO,"FAIL: enable root reclient\n");
					return FALSE;
				}
				KGLogPrintf(LOG_INFO,"enable reclient: root\n");
			}
		}
		break;
	case oneclient_gateway:
		if (!gs_timerGateway)
		{
			...
			if (gatewayinterval > 0)
			{
				...
				if (!gs_timerGateway)
				{
					KGLogPrintf(LOG_INFO,"FAIL: enable gateway reclient\n");
					return FALSE;
				}
				KGLogPrintf(LOG_INFO,"enable reclient: gateway\n");
			}
		}
		break;
	case oneclient_db:
		if (!gs_timerDB)
		{
			...
			if (dbinterval > 0)
			{
				...
				if (!gs_timerDB)
				{
					KGLogPrintf(LOG_INFO,"FAIL: enable db reclient\n");
					return FALSE;
				}
				KGLogPrintf(LOG_INFO,"enable reclient: db\n");
			}
		}
		break;
	default:
...
BOOL gReOneClient(ONECLIENT ct)
{
	...
	case oneclient_root:
		...
		KGLogPrintf(LOG_INFO,"try reclient: root - %s\n", addrRoot.c_str());

		if (!g_RootClient.Startup(freebufferRoot, buffersizeRoot, addrRoot.c_str(), portRoot))
		{
			KGLogPrintf(LOG_INFO,"FAIL: reclient root - %s\n", addrRoot.c_str());
			//return FALSE;
		}
		}}
		break;
	...
	case oneclient_gateway:
		...
		KGLogPrintf(LOG_INFO,"try reclient: gateway - %s\n", addrGateway.c_str());

		if (!g_GatewayClient.Startup(freebufferGateway, buffersizeGateway, addrGateway.c_str(), portGateway))
		{
			KGLogPrintf(LOG_INFO,"FAIL: reclient gateway - %s\n", addrGateway.c_str());
			//return FALSE;
		}
		}}
		break;
	...
	case oneclient_db:
		...
		KGLogPrintf(LOG_INFO,"try reclient: db - %s\n", addrDB.c_str());

		if (!g_DBClient.Startup(freebufferDB, buffersizeDB, addrDB.c_str(), portDB))
		{
			KGLogPrintf(LOG_INFO,"FAIL: reclient db - %s\n", addrDB.c_str());
			//return FALSE;
		}
		}}
		break;
...
BOOL gStartFriendTimingUpdateDB(UINT elapse)
{
	...
	//if (!gs_timerFriendUdtDB)
	//{
	//	KGLogPrintf(LOG_INFO,"FAIL: start friend timing update db\n");
	//	return FALSE;
	//}

	KGLogPrintf(LOG_INFO,"friend timing update db: start\n");
...
BOOL gStopFriendTimingUpdateDB()
{
	...
	KGLogPrintf(LOG_INFO,"friend timing update db: stop\n");
...
BOOL gFriendTimingUpdateDB()
{
	...
	{
		KGLogPrintf(LOG_INFO,"warning: update friend DB fail\n");
		return FALSE;
	}

	KGLogPrintf(LOG_INFO,"update friend DB succeed\n");
	return TRUE;
}
